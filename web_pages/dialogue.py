...
with open('prompt.yaml', 'r', encoding='utf-8') as file:
    data = list(yaml.safe_load(file)['prompts'].values())
...
def init_all_steps():
    all_messages = [Markdown("进行中", in_expander=True, expanded=False, title=data[0]["title"])]
    for i in range(1, len(data)):
        all_messages.append(Markdown("等待中...", in_expander=True,
                                     expanded=False, title=data[i]["title"]))
    return all_messages
